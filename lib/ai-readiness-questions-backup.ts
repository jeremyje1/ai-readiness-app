// AI Readiness Questions TypeScript structure for the AI app
// This can be imported directly into your AI app components

export interface AIReadinessQuestion {
  id: string;
  section: string;
  prompt: string;
  type: 'likert' | 'text' | 'numeric' | 'upload' | 'boolean';
  required: boolean;
  helpText?: string;
...
export const AI_DOMAINS: Record<string, Domain> = {
  ...
  organizational_readiness: {
    ...
    questions: ["AIR_18", "AIR_19", "AIR_20", "AIR_21", "AIR_22"],
    weight: 0.15
  },
  compliance_risk: {
    name: "Compliance & Risk Management",
    description: "Regulatory compliance, risk assessment, and governance",
    questions: ["AIR_23", "AIR_24", "AIR_25"],
    weight: 0.10
  }
};
...
export const AI_TIERS: Record<string, Tier> = {
  "higher-ed-ai-pulse-check": {
    ...
    description: "Quick AI readiness assessment for higher education institutions",
    questionCount: 50,
    price: 299
  },
  "ai-readiness-comprehensive": {      
    name: "AI Readiness Comprehensive",
    description: "Comprehensive AI readiness evaluation with detailed insights",
    questionCount: 105,
    price: 999
  },
  "ai-transformation-blueprint": {
    name: "AI Transformation Blueprint", 
    description: "Strategic AI implementation roadmap with detailed planning",
    questionCount: 150,
    price: 2499
  },
  "ai-enterprise-partnership": {
    name: "AI Enterprise Partnership",
    description: "Ongoing AI partnership with continuous support and optimization",
    questionCount: 200,
    price: 9999
  }
};
...
export const SCORING_CONFIG = {
  scale: {
    1: "Strongly Disagree / Not at all ready",
    2: "Disagree / Minimally ready", 
    3: "Neutral / Somewhat ready",
    4: "Agree / Mostly ready",
    5: "Strongly Agree / Fully ready"
  },
  interpretation: {
    "0-20": "Beginning - Significant AI readiness development needed",
    "21-40": "Developing - Some AI foundations in place, major gaps remain",
    "41-60": "Progressing - Moderate AI readiness with room for improvement",
    "61-80": "Advanced - Strong AI readiness with minor enhancements needed",
    "81-100": "Exemplary - Comprehensive AI readiness across all domains"
  }
};

// Helper function to get questions for a specific tier
export function getQuestionsForTier(tier: string): AIReadinessQuestion[] {
  const tierConfig = AI_TIERS[tier];
  if (!tierConfig) return AI_READINESS_QUESTIONS;
  
  // Return appropriate number of questions based on tier
  return AI_READINESS_QUESTIONS.slice(0, Math.min(tierConfig.questionCount, AI_READINESS_QUESTIONS.length));
}
  ...
  Object.entries(AI_DOMAINS).forEach(([domainKey, domain]) => {
    ...
    if (domainResponses.length > 0) {
      ...
      domainScores[domainKey] = {
        score: averageScore,
        percentage: Math.round((averageScore - 1) / 4 * 100) // Convert 1-5 scale to 0-100%
      };
    } else {
      domainScores[domainKey] = { score: 0, percentage: 0 };
    }
  });
...
// Helper function to calculate overall AI readiness score
export function calculateOverallScore(domainScores: Record<string, { score: number; percentage: number }>): number {
  ...
  Object.entries(AI_DOMAINS).forEach(([domainKey, domain]) => {
    ...
    if (score && score.percentage > 0) {
      weightedSum += score.percentage * domain.weight;
