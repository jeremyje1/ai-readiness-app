...
export class VirusScanner {
    ...
    private async analyzeFileStructure(buffer: Buffer): Promise<ThreatDetection[]> {
        ...
        try {
            ...
            if (buffer.length > 4 && !buffer.subarray(0, 4).equals(new Uint8Array(Buffer.from('%PDF')))) {
    ...
    private containsExecutableSignatures(buffer: Buffer): boolean {
        ...
        for (const signature of exeSignatures) {
            const sigStr = signature.map(b => String.fromCharCode(b)).join('');
            if (buffer.toString('binary').includes(sigStr)) {
                return true;
            }
        }
    ...
    static validateFileType(buffer: Buffer, expectedMimeType: string): boolean {
        ...
        return expectedSignatures.some(signature => {
            const bufferSlice = buffer.subarray(0, signature.length);
            const signatureArray = new Uint8Array(signature);
            return bufferSlice.length === signatureArray.length &&
                bufferSlice.every((byte, index) => byte === signatureArray[index]);
        });
    }
